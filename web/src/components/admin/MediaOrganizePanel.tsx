...
import { Check, Eye, FileText, Film, ListChecks, MoreHorizontal, Pencil, Plus, RefreshCw, Save, Square, Trash2, X } from "lucide-react"
...
export function MediaOrganizePanel() {
  ...
  const load = async () => {
    setMessage("")
    try {
      const [accountRes, mediaRes, mediaSettingsRes] = await Promise.allSettled([
        adminApi.accounts(),
        adminApi.mediaTasks(),
      ...
      const text = getMessage(err, "媒体整理信息加载失败")
      setMessage(text)
      toast.error(text)
    }
  }
  ...
  return (
    <div className="grid gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-semibold">媒体整理</h2>
        </div>
